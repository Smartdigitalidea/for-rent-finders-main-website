...
const BlogSidebar = ({
    ...
    const fetchPosts = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('blog_posts').select('id, title, excerpt, image_url, category, read_time, published_at, featured').eq('status', 'published').neq('id', currentPostId || '').order('published_at', {
          ascending: false
        }).limit(6);
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        {posts.map(post => <Link key={post.id} to={`/blog/${post.id}`} className="block group">
            <article className="bg-card border border-border rounded-lg p-4 hover:shadow-md transition-all duration-200 hover:border-primary/20">
              <div className="flex gap-3">
                <div className="flex-shrink-0">
                  <img src={post.image_url} alt={post.title} className="w-16 h-16 object-cover rounded-lg" onError={e => {
      ...
      <div className="border border-border rounded-lg p-4">
        ...
        <div className="space-y-2">
          <input type="email" placeholder="Enter your email" className="w-full px-3 py-2 text-sm border border-border rounded-md focus:outline-none focus:ring-1 focus:ring-primary" />
