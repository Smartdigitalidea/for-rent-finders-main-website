...
const Header = () => {
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 luxury-card bg-card/95 backdrop-blur-xl border-b border-border/50">
      {/* Top contact bar */}
      <div className="bg-gradient-to-r from-primary to-orange-500 text-white py-2">
        <div className="container mx-auto px-4 flex justify-center items-center space-x-6 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <a href="tel:8553677368" className="hover:text-orange-200 transition-colors">855-FOR-RENT </a>
          </div>
          <div className="flex items-center space-x-2">
      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center py-4">
          ...
          <nav className="hidden md:flex space-x-8">
            {menuItems.map(item => <Link key={item.name} to={item.href} className="text-gray-700 hover:text-orange-500 font-medium transition-colors">
                {item.name}
              </Link>)}
          </nav>
        ...
        {isMenuOpen && <div className="md:hidden py-4 border-t">
            <nav className="flex flex-col space-y-4">
              {menuItems.map(item => <Link key={item.name} to={item.href} className="text-gray-700 hover:text-orange-500 font-medium" onClick={() => setIsMenuOpen(false)}>
                  {item.name}
                </Link>)}
